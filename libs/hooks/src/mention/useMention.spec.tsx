...
describe("useMention", () => {
  ...
  it("should work!", async () => {
    ...
    expect(result.current.Mention.props.autoFocus).toEqual(undefined);

    // updateConfig
    result.current.updateConfig({ autoFocus: true });
    expect(result.current.Mention.props.autoFocus).toEqual(true);

    // updateSearchUserQuery
    ...
    result.current.Mention.props.onSearch("a", "prefix");
    rerender();
    ...
    result.current.Mention.props.onSearch("", "prefix");
    rerender();
    await jest.advanceTimersByTime(400);
    await act(async () => {
      await (global as any).flushPromises();
